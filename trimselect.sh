...
dirs=$(ls $fastadir | grep -e "S2")

for dir in $dirs; do
	
	mkdir -p $trimdir/$dir

	samples=$(ls $fastadir/$dir | rev | cut -d '.' -f2 | rev | cut -d '_' -f2 | sort -u)
	...
	for name in $samples; do
		...
		java -jar -Xmx50G $trimmomatic PE -threads 100 -phred33 -trimlog $logdir/${name}_trim.log \
